...
Clarinet.test({
  name: "[set-provenance-hash] Contract owner can set provenance hash properly",
  async fn(chain: Chain, accounts: Map<string, Account>) {
...
Clarinet.test({
  name: "[set-provenance-hash] Contract owner can set provenance hash only once",
  async fn(chain: Chain, accounts: Map<string, Account>) {
    let wallet_deployer = accounts.get("deployer")!;
    let block = chain.mineBlock([
      Tx.contractCall(
    ...
    block.receipts[1].result.expectErr().expectUint(16);
  },
});
